...
- (instancetype)initWithTitle:(NSArray *)title iconsName:(NSArray *)iconNames{
    ...
    if (self) {
        ...
        CGFloat itemX = 10;
        NSUInteger count = title.count <= iconNames.count ? title.count:iconNames.count;
        for (int i = 0; i < count ; i++) {
            ActionSheetItem *item = [[ActionSheetItem alloc] initWithFrame:CGRectMake(itemX, 30, 60, 110)];
            item.btnIndex = i;
            [item setTitle:title[i] image:[UIImage imageNamed:iconNames[i]]];
...
- (void)clickAction:(ActionSheetItem *)item
{
    if (_clickBlock) {
